...
- **单人模式** — 本地运行，与 AI 对手即时对战；已登录用户每局随机加入最多 2 个 LLM 大模型机器人（DeepSeek/Kimi/MiniMax/Qwen）和最多 2 个具名规则机器人（Blaze/Shield/Sage），其余为匿名规则引擎机器人
...
单人模式只需启动客户端即可游戏（规则引擎机器人不需联网），但若需要 LLM 大模型机器人加入，则需同时启动服务端并登录。多人在线需要同时启动服务端。开发模式下支持局域网 IP 访问（手机等设备通过 `http://局域网IP:5173` 即可连接，CORS 和 API 地址自动适配）。
